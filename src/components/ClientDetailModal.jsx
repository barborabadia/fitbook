...
export default function ClientDetailModal({ client, onClose, onDelete, onMerge }) {
  ...
  const isGroupCash = name => name?.includes('Zbůch') || (name?.includes('Březín') && !name?.includes('Tabata') && !name?.includes('Charitativní')) || name?.includes('Holýšov') || name?.includes('Nýřany')
