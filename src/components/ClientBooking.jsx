...
const ICONS = { 'Hubneme': '❤️', 'Charitativní': '❤️', 'XXL': '🏆', 'Funkční': '⚡', 'Osobní': '💪', 'Posilování': '🎵', 'Orient': '🥋', 'Tabata': '🔥', default: '💪' }
...
const s = {
  ...
  tab: (active) => ({ flex: '1 1 calc(50% - 4px)', textAlign: 'center', padding: '8px 10px', borderRadius: 10, border: `1px solid ${active ? 'rgba(200,81,107,0.3)' : '#EBCFD8'}`, background: active ? 'rgba(200,81,107,0.08)' : '#FFFFFF', color: active ? '#C8516B' : '#9B7E8A', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.15s' }),
...
const CERTIFIKATY = [
  ...
  'Sestavování fitness tréninku.pdf',
  'Tejpování.pdf',
            ...
            {preview ? (
              <span>
                <span
                  ...
                  style={{ color: '#C8516B', cursor: 'pointer', fontSize: 14, fontWeight: 600, fontFamily: 'inherit', marginRight: 10 }}
                >← Zpět</span>
                {preview.replace('.pdf', '')}
              </span>
            ) : '🎓 Certifikáty'}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {CERTIFIKATY.map(name => (
              <button
                ...
                style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', borderRadius: 10, border: '1px solid #EBCFD8', background: 'transparent', color: '#2C1A22', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left', transition: 'background 0.15s' }}
