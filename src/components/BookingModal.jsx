...
function getPrice(slot, bookingType) {
  ...
  if (name.includes('Nýřany')) return 135
...
export default function BookingModal({ slot, prefill, onClose }) {
  ...
  const isZbuch = ((slot?.name || '').includes('Březín') && !(slot?.name || '').includes('Tabata') && !(slot?.name || '').includes('Charitativní')) || (slot?.name || '').includes('Holýšov') || isDonationBased(slot?.name)
  const isSep = (slot?.slot_date || '') >= '2026-09-01'
  ...
  async function checkUnpaid(email) {
    ...
    const isCashTraining = n => (n?.includes('Březín') && !n?.includes('Tabata') && !n?.includes('Charitativní')) || n?.includes('Holýšov') || isDonationBased(n)
                  ...
                  <>
                    ...
                    <div style={s.typeRow}>
                      <div style={s.typeCard(bookingType === 'solo', '#C8516B')} onClick={() => setBookingType('solo')}>
                        ...
                        <div style={s.typePrice}>{isSep ? 300 : 200} Kč / 60 min</div>
                      </div>
                      <div style={s.typeCard(bookingType === 'duo', '#C8516B')} onClick={() => setBookingType('duo')}>
                        ...
                        <div style={s.typePrice}>{isSep ? 400 : 300} Kč / 60 min</div>
                      </div>
                    </div>
                  </>
                )}
