...
const s = {
  ...
  form: { background: '#111118', border: '1px solid #1E1E2E', borderRadius: 16, padding: 24, marginBottom: 28 },
  ...
  label: { fontSize: 11, fontWeight: 600, color: '#555', textTransform: 'uppercase', letterSpacing: '0.8px', display: 'block', marginBottom: 6 },
  input: { width: '100%', background: '#0A0A0F', border: '1px solid #1E1E2E', borderRadius: 10, padding: '11px 14px', color: '#F0EDE8', fontSize: 14, fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box' },
  select: { width: '100%', background: '#0A0A0F', border: '1px solid #1E1E2E', borderRadius: 10, padding: '11px 14px', color: '#F0EDE8', fontSize: 14, fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box' },
  table: { background: '#111118', border: '1px solid #1E1E2E', borderRadius: 16, overflow: 'hidden' },
  tHead: { display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 60px', padding: '12px 24px', fontSize: 11, fontWeight: 700, color: '#444', textTransform: 'uppercase', letterSpacing: '0.8px', borderBottom: '1px solid #1E1E2E' },
  tRow: { display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 60px', padding: '14px 24px', borderBottom: '1px solid #0F0F1A', alignItems: 'center', fontSize: 14 },
  catBadge: (cat) => ({ display: 'inline-block', padding: '3px 10px', borderRadius: 20, fontSize: 11, fontWeight: 700, background: `${CATEGORY_COLORS[cat] || '#666'}18`, color: CATEGORY_COLORS[cat] || '#666', border: `1px solid ${CATEGORY_COLORS[cat] || '#666'}33` }),
  deleteBtn: { background: 'none', border: 'none', color: '#333', cursor: 'pointer', fontSize: 16, padding: '4px 8px', borderRadius: 6, transition: 'color 0.15s' },
  empty: { padding: '40px 24px', textAlign: 'center', color: '#444', fontSize: 14 },
    ...
    <div>
      ...
      <div style={s.summaryRow}>
        <div style={s.stat}>
          ...
          <div style={{ ...s.statValue, color: '#FF4D00' }}>{totalExpenses.toLocaleString('cs-CZ')} Kč</div>
        ...
        <div style={s.stat}>
          ...
          <div style={{ ...s.statValue, fontSize: topCategory ? 20 : 28, color: topCategory ? CATEGORY_COLORS[topCategory[0]] : '#444' }}>
      ...
      <div style={s.form}>
        ...
        <div style={s.formGrid}>
          <div>
            ...
            <input
              style={s.input}
              placeholder="např. gumy na cvičení, pronájem sálu..."
        ...
        <button
          style={{ ...s.btn('primary'), marginTop: 16, opacity: saving ? 0.6 : 1 }}
          onClick={addExpense}
          disabled={saving}
        >
      ...
      <div style={s.table}>
        ...
        {expenses.map((e, i) => (
          <div key={e.id} style={{ ...s.tRow, background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.01)' }}>
            <span style={{ fontWeight: 600 }}>{e.description}</span>
            <span><span style={s.catBadge(e.category)}>{e.category}</span></span>
            <span style={{ color: '#666', fontSize: 13 }}>{formatDate(e.expense_date)}</span>
            <span style={{ fontWeight: 700, color: '#FF4D00' }}>{e.amount.toLocaleString('cs-CZ')} Kč</span>
            <button
              style={s.deleteBtn}
