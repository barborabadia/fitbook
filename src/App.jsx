...
const s = {
  ...
  navItem: (active) => ({ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', borderRadius: 10, cursor: 'pointer', fontSize: 14, fontWeight: active ? 600 : 400, color: active ? '#C8516B' : '#A08090', background: active ? 'rgba(200,81,107,0.07)' : 'transparent', border: active ? '1px solid rgba(200,81,107,0.15)' : '1px solid transparent', marginBottom: 4 }),
  linkBox: { margin: '0 12px 12px', background: '#FBF6F8', border: '1px solid #EBCFD8', borderRadius: 12, padding: '12px 14px' },
      ...
      <div style={s.sidebar}>
        ...
        <nav style={s.nav}>
          {[
            ...
            { id: 'expenses', label: 'Náklady', icon: '💸' },
          ].map(item => (
        ...
        <div style={s.linkBox}>
          ...
          <img
            src={`https://api.qrserver.com/v1/create-qr-code/?size=160x160&data=${encodeURIComponent(bookingUrl)}&bgcolor=FBF6F8&color=C8516B&margin=8`}
            alt="QR kód"
        ...
        <div style={{ margin: '0 12px 20px' }}>
          <button onClick={() => { sessionStorage.removeItem('fitbook_admin'); setIsLoggedIn(false) }} style={{ width: '100%', padding: '8px', borderRadius: 8, border: 'none', background: '#F5EEF1', color: '#C4ABB4', cursor: 'pointer', fontSize: 12, fontWeight: 600, fontFamily: 'inherit' }}>Odhlásit se</button>
        </div>
      </div>
      <div style={s.main}>
