...
export default function App() {
  ...
  const [isLoggedIn, setIsLoggedIn] = useState(
    sessionStorage.getItem('fitbook_admin') === '1'
  )

  if (isClientView) {
    return <div style={s.root}><ClientBooking /></div>
  }
    ...
    <div style={s.root}>
      <div style={s.sidebar}>
        ...
        <div style={{ margin: '0 12px 20px' }}>
          <button
            onClick={() => { sessionStorage.removeItem('fitbook_admin'); setIsLoggedIn(false) }}
            style={{
              width: '100%', padding: '8px', borderRadius: 8, border: 'none',
              background: '#1A1A28', color: '#555', cursor: 'pointer',
      ...
      {selectedTraining && (
        <BookingModal training={selectedTraining} prefill={null} onClose={() => setSelectedTraining(null)} />
      )}
    </div>
  )
}
